...
const privateIpRange = new gcp.compute.GlobalAddress(
  `${resourcePrefix}gotloop-private-ip`,
  {
    purpose: 'VPC_PEERING',
...
export const vpcPeering = new gcp.servicenetworking.Connection(
  `${resourcePrefix}gotloop-vpc-peering`,
  {
    network: network.id,
...
export const vpcEgressSubnet = new gcp.compute.Subnetwork(
  `${resourcePrefix}gotloop-vpc-egress`,
  {
